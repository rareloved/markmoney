var rygj_query = {
	...
	addSpan : function(data, id, lx) {
		// this.html += '<span class="bg-info" id='+id+'
		// lx='+lx+'>'+data+'</span>';
		this.html += ' <button type="button" class="btn btn-default " id=' + id
				+ ' lx=' + lx + ' style="width: 240px;">' + data + '</button>';
	},
	addRightSpan : function(data, id, lx) {
		// this.html += '<span class="bg-info" id= '+id+' lx ='+lx+'
		// style="float:right;">'+data+'</span>';
		this.html += ' <button type="button" class="btn btn-default  pull-right" id='
				+ id
				+ ' lx='
				+ lx
				+ ' style="width: 240px;">'
	...
	addContent : function(data) {
		...
		for (i in data) {
			...
			if (this.sfzh[0] == data[i].sfzh) {
				...
				this.addPartStart();
				this.addSpan(this.getMessageFromData(data[i]), data[i].xxzjbh,
						lx);
				pref_lx = lx
				pref_date = date
			} else {
				...
				if (pref_date == date && pref_lx == lx) {
					// 轨迹日期相等且轨迹类型相等(同轨迹)时
					this.addRightSpan(this.getMessageFromData(data[i]),
							data[i].xxzjbh, lx);
				} else {
					// 不同轨迹时
					this.addLineStart(date);
	...
	detail : function(e) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				for ( var key in data) {
					var str = "MAP."+key;
					tab+="<tr><td>"+eval(str)+"</td><td>"+data[key]+"</td></tr>"
				}
				tab += "</table>";
				$(".modal-body p").html(tab);
				$("#mymodal").modal("show");
			}
		});
	}
